...
#   }
#
#   # Continuous endpoint logic implementation
#   message("[plot_exposure_response] Detected continuous response variable — using standard plotting logic.")
#   return(plot_exposure_response_continuous(
#     data = data,
#     dose_var = as.character(substitute(dose_var)),
#     exposure_var = as.character(substitute(exposure_var)),
...
#     effect_label_position = effect_label_position,
#     vertical_line = vertical_line
#   ))
# }
#
...
#
#
#   # Add response bin annotations (N, n, rate)
#   add_response_annotations <- function(summary_df) {
#     annotate_df <- summary_df %>%
#       mutate(
#         ypos_n = 1.05,
#         ypos_nresp = 1.10,
#         ypos_rate = 1.15,
#         label_group = ifelse("Endpoint" %in% names(.), Endpoint, ifelse("Group" %in% names(.), Group, NA))
#       )
#
#     list(
#       geom_text(data = annotate_df, aes(x = midptx, y = ypos_n, label = N, color = label_group), size = 3.1, inherit.aes = FALSE, show.legend = FALSE),
#       geom_text(data = annotate_df, aes(x = midptx, y = ypos_nresp, label = n, color = label_group), size = 3.1, inherit.aes = FALSE, show.legend = FALSE),
#       geom_text(data = annotate_df, aes(x = midptx, y = ypos_rate, label = rate_label, color = label_group), size = 3.1, inherit.aes = FALSE, show.legend = FALSE),
#       annotate("text", x = max(annotate_df$midptx) * 1.05, y = 1.05, label = "N", size = 3.1),
#       annotate("text", x = max(annotate_df$midptx) * 1.05, y = 1.10, label = "n", size = 3.1),
#       annotate("text", x = max(annotate_df$midptx) * 1.05, y = 1.15, label = "Rate", size = 3.1)
#     )
#   }
#
#
...
#       add_response_annotations(summary_df)
#
...
#     coord_cartesian(xlim = xlims, ylim = c(0, 1.25)) +
#     add_response_annotations(summary_df) +
#     theme_minimal() +
#     { if (is.null(group_var)) theme(legend.position = "none") } +
#     {
