...
simulate_emax <- function(dose_levels, n_per_dose, e0, emax, ed50, sigma=0,
  ...
  grid <- expand.grid(e0 = e0, emax = emax, ed50 = ed50, KEEP.OUT.ATTRS = FALSE, stringsAsFactors = FALSE)
  result_list <- vector("list", length = nrow(grid))

  for (i in seq_len(nrow(grid))) {
    pars <- grid[i, ]
    dose_vector <- rep(dose_levels, times = n_per_dose)
    pred <- pars$e0 + pars$emax * dose_vector / (pars$ed50 + dose_vector)
    n <- length(pred)
    residual <- switch(
      tolower(dist),
    ...
    ipred <- pred + residual
    label <- paste0("e0=", signif(pars$e0, 3), ", emax=", signif(pars$emax, 3), ", ed50=", signif(pars$ed50, 3))
